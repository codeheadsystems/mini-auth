...
import com.codeheadsystems.miniconsole.server.http.Json;
...
/**
 * The served OpenAPI 3.1 spec for the console's small read-only {@code /api} JSON surface, loaded once
 * from the checked-in {@code /openapi.yaml} resource.
 *
 * <p>The YAML file is the single authoritative contract. We serve it verbatim at {@code /openapi.yaml}
 * and — because some tools prefer JSON — parse it once and re-emit it as JSON for {@code /openapi.json}.
 * Both come from the same source, so the two representations can never disagree. The contract test
 * parses these bytes and checks the documented paths against the live routes. A verbatim copy of the
 * family's {@code OpenApiDocument}.
 */

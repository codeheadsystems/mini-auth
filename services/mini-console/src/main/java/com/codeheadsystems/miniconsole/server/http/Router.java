 ...
 * Patterns use {@code {name}} segments (e.g. {@code /identities/{id}}); a matched variable is
 * exposed via {@link RequestContext#pathParam}. First registered match wins; a path that matches but
 * with the wrong method yields 405, an unmatched path yields 404.
 *
 ...
 * logging records method, path, and status only; request/response bodies (which may carry secrets,
 * e.g. the pasted console token) are never logged.
 *
 * <p>This is a verbatim copy of mini-oidc's {@code server/http/Router} (a documented
 * {@code mini-common} candidate), kept so the console is self-contained.
 */
public final class Router implements HttpHandler {
  ...
  private static void write(final HttpExchange exchange, final HttpResponse response) throws IOException {
    ...
    // Extra headers (Location for redirects; one or more Set-Cookie) are ADDED, not set, so
    // multiple values (e.g. several cookies) are all emitted.
    for (final HttpResponse.Header header : response.headers()) {
      exchange.getResponseHeaders().add(header.name(), header.value());
    }
    // 204/302 and empty bodies must not declare a body length; everything else sends its byte count.
